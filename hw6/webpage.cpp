...
#include <set>
	...
	void WebPage::set_incoming(std::set<std::string> incoming){
		incoming_links = incoming;
	}
	void WebPage::set_outgoing(std::set<std::string> outgoing){
		link_to_file = outgoing;
	}
	//bool WebPage::lookup();
	std::set<std::string> WebPage::getlink(){
		return link_to_file;
	}
	std::set<std::string> WebPage::get_incoming(){
