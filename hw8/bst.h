...
Node<Key,Value>* BinarySearchTree<Key,Value>::removeHelper(Node<Key,Value>* root,const Key& key)
{
    ...
    else
    {
        ...
        else if (root->getRight() == NULL)
        {
        	...
        	Node<Key,Value>* predecessor = maxHelper(root->getLeft());
 			Node<Key,Value>* temp1 = new Node<Key,Value>(predecessor->getKey(),predecessor->getValue(),
 														root->getParent());
 			temp1->setLeft(root->getLeft());
 			...
 			temp1->setHeight(root->getHeight());
			if(temp1->getParent()!=NULL){
				if(temp1->getParent()->getLeft()==NULL && temp1->getParent()->getRight()==NULL){
					delete root;
					root = temp1;
					return root;
				}
				else if(temp1->getParent()->getLeft()==NULL){
					temp1->getParent()->setRight(temp1);
				}
				else if(temp1->getParent()->getRight()==NULL){
					temp1->getParent()->setLeft(temp1);
				}
				else{
					if(root->getKey()==temp1->getParent()->getLeft()->getKey()){
						temp1->getParent()->setLeft(temp1);
					}
					else{
						temp1->getParent()->setRight(temp1);
        	...
        	root->setLeft(removeHelper(root->getLeft(), predecessor->getKey()));
